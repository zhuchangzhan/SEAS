...
def get_HITRAN_ID():
    ...
    file = open("../../SEAS_Input/Line_List/HITRAN_Molecule_List.txt").read().split("\n")
    ID = []
...
class HITRAN_Line_List_downloader():
    ...
    def __init__(self,
        ...
        self.path = os.path.join(outpath,self.n)
    ...
    def download(self):
        ...
        try:
            ...
            with open(os.path.join(self.outpath,"download.log"),"a") as f:
                f.write("%s success\n"%self.n)
        except:
            with open(os.path.join(self.outpath,"download.log"),"a") as f:
                f.write("%s failed\n"%self.n)
